...
輸出檔案：
- models/artifacts/lstm.pt (模型權重)
- models/artifacts/lstm_scaler.json (正規化參數)
- evaluation/results/lstm_training_log.csv (訓練日誌)
- evaluation/results/lstm_training_report.txt (訓練報告)
- evaluation/results/lstm_training_curves.png (訓練曲線圖)
...
class TrainingLogger:
    ...
    def __init__(self, output_dir: Path, prefix: str = "lstm"):
    ...
    def save_csv(self):
        ...
        csv_path = self.output_dir / f"{self.prefix}_training_log.csv"
...
--------------------------------------------------------------------------------
1. 使用雙向 LSTM 捕捉前後文信息
2. Attention 機制聚焦重要時間步
3. 多層結構學習複雜時序模式
4. Dropout 和 Layer Normalization 防止過擬合

================================================================================
"""
        report_path = self.output_dir / f"{self.prefix}_training_report.txt"
    ...
    def plot_curves(self):
        ...
        try:
            ...
            plt.tight_layout()
            plot_path = self.output_dir / f"{self.prefix}_training_curves.png"
...
def main():
    ...
    logger = TrainingLogger(output_dir, prefix="lstm")
    ...
    best_val_acc = 0
    patience_counter = 0
    model_save_path = PROJECT_ROOT / "models" / "artifacts" / "lstm.pt"
    ...
    # 儲存 scaler 參數
    scaler_path = PROJECT_ROOT / "models" / "artifacts" / "lstm_scaler.json"
