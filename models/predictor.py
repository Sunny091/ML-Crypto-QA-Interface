"""
Predictor Module - 模型預測服務

Provides prediction classes for loading and using trained models.
Supports price-only and price+text models.
"""
...
from etl.transform import PRICE_ONLY_FEATURES, PRICE_TEXT_FEATURES
    ...
    def load(self) -> None:
        ...
        if not self.model_path.exists():
            raise FileNotFoundError(
                ...
                "Run 'python -m models.training.train' to create it."
            )
...
class PriceTextPredictor:
    ...
    def load(self) -> None:
        ...
        if not self.model_path.exists():
            raise FileNotFoundError(
                f"Model file not found: {self.model_path}. "
                "Run 'python -m models.training.train --with-text' to create it."
            )
